...
var resizePizzas = function(size) {
  ...
  // Iterates through random pizza container elements on the page and changes their widths
  function changePizzaSizes(size) {
    var randomPizzaContainer = document.getElementsByClassName("randomPizzaContainer");
    var numberOfPizza = randomPizzaContainer.length;
...
var pizzasDiv = document.getElementById("randomPizzas");  //variable declaration was moved outside for loop to avoid repeated defination.
for (var i = 2; i < 100; i++) {
...
function updatePositions() {
  ...
  var cols = Math.floor(maxWidth/s) + 1;
  var s = 256;
...
// Generates the sliding pizzas when the page loads. Number of pizza just enough to fill whole viewport. Drawback is they won't fit when resizing window untill refresh page.
document.addEventListener('DOMContentLoaded', function() {
  var maxWidth = document.documentElement.clientWidth;
  var maxHeight = document.documentElement.clientHeight;
  // var maxWidth = window.screen.width;
  // var maxHeight = window.screen.height; These does not work properly for phones. I don't quite understand why.
  var s = 256;
  var cols = Math.floor(maxWidth/s) + 1;
  var row = Math.floor(maxHeight/s) + 1;
